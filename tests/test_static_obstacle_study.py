...
# Other params
i_T = 0           # Do not change
i_dt = 1          # Do not change

i_u_offsets = 0   # Do not change
i_chi_offsets = 1 # Do not change

i_K_e = 15        # Do not change

value_T = 80      # Standard value 120
value_dt = 1.0    # Standard value: 1.0
value_K_e = 0     # Standard value:   

u_offsets = [np.array([1, 0.5])]
chi_offsets = [np.array([
        -220, -200, -180, -160, -140, -120, -110, -100, -80, -60.0, -40, -20, 0, 20, 40, 60, 80, 100, 110, 120, 140, 160, 180, 220])]
...
value_d_safe = 40       # Standard value: 4.5
value_G1 = 100          # Standard value: 100.0
# value_G2 = 5.0        # Standard value: 5.0
value_G3 = 0.18         # Standard value: 1.4    # Tune val. : 0.14
value_G4 = 0.01         # Standard value: 1.0    # Tune val. : 0.35
...
#
LOS_lookahead_distance = 130
colav_config.layer1.psbmpc.ownshipparams.set_LOS_LD(LOS_lookahead_distance)

# Setting the parameters in the PSBMPC
colav_config.layer1.psbmpc.psbmpcparams.set_par_double(index = i_T, value = value_T)
colav_config.layer1.psbmpc.psbmpcparams.set_par_double(index = i_dt, value = value_dt)
colav_config.layer1.psbmpc.psbmpcparams.set_par_vector(index = i_u_offsets, values = u_offsets)
colav_config.layer1.psbmpc.psbmpcparams.set_par_vector(index = i_chi_offsets, values = chi_offsets)
...
for i in range(len(os_x_data)):
    ...
    #if i%75 == 0: # Plotting functionality
    #    simulator.enc.start_display()
    #    simulator.enc.add_vessels((0, int(os_x_data[i]), int(os_y_data[i]), 30, "black"))
    #    try:
    #        simulator.enc.draw_circle([int(os_x_data[i]), int(os_y_data[i])], d_data[i], "magenta", fill = True, alpha = 0.6)
    #    except:
...
path = '/mnt/c/Users/JeMyk/OneDrive/Dokumenter/NTNU/Industriell kybernetikk/3 - Haust 2023/Prosjektoppgåve/SimData/'
